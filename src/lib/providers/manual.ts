...
import type { CruiseProvider, ProviderItinerary, ProviderSailing, ProviderSailingSearch } from "./types";
...
export const manualProvider: CruiseProvider = {
	...
	async searchSailings(query: ProviderSailingSearch): Promise<ProviderSailing[]> {
		const itineraries = await prisma.itinerary.findMany({
			where: {
				source: "MANUAL",
				...(query.departurePort ? { departurePort: { contains: query.departurePort, mode: "insensitive" } } : {}),
