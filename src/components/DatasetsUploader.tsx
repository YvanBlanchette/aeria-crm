...
export default function DatasetsUploader() {
  ...
  async function downloadDataset(dataset: DatasetItem) {
    ...
    try {
      const res = await fetch(`/api/datasets?slug=${encodeURIComponent(dataset.slug)}&raw=1`, {
        cache: "no-store",
      });
      const data = (await res.json()) as {
        ok?: boolean;
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-xs text-slate-500 mt-1">
          ou cliquez pour selectionner un ou plusieurs fichiers
        </p>
      </label>
      ...
      {canForceSlug && (
        <div>
          <label htmlFor="dataset-forced-slug" className="label">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Laissez vide pour deduire automatiquement depuis le nom du fichier.
          </p>
        </div>
      )}
                ...
                {datasets.map((dataset) => (
                  <tr key={dataset.slug} className="border-t border-slate-100">
                    ...
                    <td className="py-1 pr-4 text-slate-600">
                      {new Date(dataset.updatedAt).toLocaleString("fr-CA")}
                    </td>
                    <td className="py-1">
                      <button
                        type="button"
