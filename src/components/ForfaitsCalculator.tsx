...
export function ForfaitsCalculator() {
	...
	const cabinCalc = useCallback(
		(b: BaseCalc, cabinePrix: number): CabinResult => {
			...
			const prixPers =
				step > 0 ? Math.ceil(brut / step) * step : brut;
	...
	const validateReady = useCallback(() => {
		...
		if (form.vols === 0 && !form.hasPre)
			issues.push("Renseignez le coût des vols");

		if (form.hasPre) {
			if (form.nuitsHotel <= 0)
				issues.push("Séjour pré: nuits > 0");
			if (form.hotel <= 0) issues.push("Séjour pré: coût hôtel > 0");
		}
		if (form.hasPost) {
			if (form.nuitsHotelPost <= 0)
				issues.push("Séjour post: nuits > 0");
			if (form.hotelPost <= 0)
				issues.push("Séjour post: coût hôtel > 0");
		}
				...
				{[1, 2, 3, 4].map((s) => (
					<button
						...
						className={`px-4 py-2 text-sm font-semibold ${
							currentStep === s
								? "text-lagon border-b-2 border-lagon"
								: "text-gray-500"
						}`}
					>
						{s === 1
							...
							>
								...
								<input
									...
									value={
										String(
											form[
												cab.key as "cabInt" | "cabExt" | "cabBal" | "cabSui"
											] ?? 0
										)
									}
									onChange={(e) =>
				...
				<div className="space-y-4">
					...
					<div className="space-y-2 border-t pt-4">
						...
						{!form.pourboiresInclus && (
							<label className="space-y-1">
								<span className="text-sm">
									Ou manuel ({form.pourboiresManuel ??
										constants.pourboiresNuit *
											form.nuits}{" "}
									$ par défaut)
								</span>
								<input
									...
									{cabs.map((cab) => (
										...
										>
											...
											<div className="text-xs text-gray-600 space-y-1">
												<div>
													Cabine: ${cab.cabinePers.toFixed(2)} +
													Frais:
													$
													{(
														base.vols +
