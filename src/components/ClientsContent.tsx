...
export function ClientsContent({
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          <Link
            ...
            className={`badge ${view === "active" ? "bg-navy text-white" : "bg-white border border-slate-300 text-slate-600"}`}
          >
            Actifs
          </Link>
          <Link
            href="/clients?view=archived"
            className={`badge ${view === "archived" ? "bg-navy text-white" : "bg-white border border-slate-300 text-slate-600"}`}
          >
            Archives
          </Link>
          <Link
            href="/clients?view=all"
            className={`badge ${view === "all" ? "bg-navy text-white" : "bg-white border border-slate-300 text-slate-600"}`}
          >
            Tous
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          />
          <a href="/api/clients/export" className="btn-secondary">
            Exporter CSV
          </a>
        </div>
      </div>
      ...
      {hasImportResult && (
        <p
          className={`text-sm rounded-lg px-4 py-3 border ${isPreview ? "text-ocean-700 bg-ocean-50 border-ocean-200" : "text-emerald-800 bg-emerald-50 border-emerald-200"}`}
        >
          {isPreview ? "Aperçu import" : "Import terminé"} : {imported} créé(s), {updated} mis à
          jour, {skipped} ignoré(s).
        </p>
      )}
