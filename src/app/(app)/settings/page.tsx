    ...
    <div className="space-y-6">
      ...
      {user.role === "ADMIN" && (
        <section className="card p-5">
          <h2 className="font-semibold text-navy mb-4">
            Profil agence &amp; règles opérationnelles
          </h2>
      ...
      <div className="grid lg:grid-cols-2 gap-6 items-start">
                ...
                <ul className="mt-1 space-y-1">
                  {line.ships.map((s) => {
                    ...
                    return (
                      <li
                        key={s.id}
                        className="flex items-center justify-between text-sm text-slate-600 pl-3"
                      >
                        <span>⛴ {s.name}</span>
                        <form action={remove}>
                          <button
        ...
        <div className="space-y-6">
                ...
                {users.map((u) => (
                  <li key={u.id} className="py-3 flex items-center justify-between gap-3 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`badge ${u.role === "ADMIN" ? "bg-navy text-white" : "bg-slate-100 text-slate-600"}`}
                      >
          ...
          <section className="card p-5">
            <h2 className="font-semibold text-navy mb-4">
              Journal d&apos;audit (paramètres & sécurité)
            </h2>
            {auditLogs.length === 0 ? (
          ...
          <section className="card p-5">
            ...
            <p className="text-sm text-slate-600">
              Le CRM est prêt à recevoir des fournisseurs d&apos;itinéraires externes (Traveltek,
              Widgety, GDS…). L&apos;architecture se trouve dans{" "}
              <code className="text-xs bg-slate-100 px-1 py-0.5 rounded">src/lib/providers/</code> :
              implémentez l&apos;interface{" "}
              <code className="text-xs bg-slate-100 px-1 py-0.5 rounded">CruiseProvider</code> et
          ...
          {user.role === "ADMIN" && (
            <section className="card p-5">
              ...
              <p className="text-sm text-slate-600 mb-4">
                Uploadez une nouvelle version d&apos;un ou plusieurs fichiers JSON pour remplacer
                les données en base immédiatement.
              </p>
              <DatasetsUploader />
            </section>
          )}
