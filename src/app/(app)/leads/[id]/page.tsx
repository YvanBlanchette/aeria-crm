        ...
        <section className="card p-5 space-y-3 lg:col-span-1">
          ...
          <dl className="text-sm space-y-2">
            ...
            <div>
              ...
              <dd>{lead.destination ?? "—"}</dd>
            </div>
            <div>
              <dt className="label">Période</dt>
              <dd>{lead.travelPeriod ?? "—"}</dd>
            </div>
            <div>
              <dt className="label">Budget</dt>
              <dd>{fmtMoney(lead.budget)}</dd>
            </div>
            <div>
              ...
              <dd>
                {lead.client ? (
                  ...
                  <>
                    {lead.contactName ?? "—"}
                    {lead.contactEmail && (
                      <span className="block text-slate-500">{lead.contactEmail}</span>
                    )}
                    {lead.contactPhone && (
                      <span className="block text-slate-500">{lead.contactPhone}</span>
                    )}
                  </>
                )}
              </dd>
        ...
        <section className="card p-5 lg:col-span-2">
          ...
          {lead.activities.length === 0 ? (
            <p className="text-sm text-slate-500">
              Aucune activité. Consignez chaque échange pour garder l&apos;historique complet.
            </p>
          ) : (
              ...
              {lead.activities.map((a) => (
                <li key={a.id} className="border-l-2 border-ocean-100 pl-4">
                  <div className="flex items-center gap-2 text-xs text-slate-500">
                    <span className="badge bg-ocean-50 text-ocean-600">
