...
export default async function Dashboard() {
  ...
  const agentIds = bookingsByAgent
    .map((b) => b.userId)
  ...
  const kpis = [
    ...
    { label: "Commissions totales", value: fmtMoney(revenueAgg._sum.commission), href: "/bookings" },
    { label: "CA confirmé du mois", value: fmtMoney(monthRevenueAgg._sum.totalPrice), href: "/bookings" },
    { label: "Commissions du mois", value: fmtMoney(monthRevenueAgg._sum.commission), href: "/bookings" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500">Cockpit agence du {fmtDate(now)}: ventes, risques et priorités du jour.</p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <section className="card p-5">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Soldes à encaisser (14 jours)</p>
          <p className="text-2xl font-bold text-amber-700 mt-1">{fmtMoney(dueAmountTotal)}</p>
          <p className="text-sm text-slate-600 mt-2">{dueBookingsWithBalance.length} réservation(s) avec échéance proche.</p>
        </section>
        <section className="card p-5">
        ...
        <section className="card p-5">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Taux de gain prospects</p>
          <p className="text-2xl font-bold text-emerald-700 mt-1">{leadWinRate === null ? "—" : `${leadWinRate}%`}</p>
          <p className="text-sm text-slate-600 mt-2">{wonLeads} gagné(s), {lostLeads} perdu(s).</p>
        </section>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <section className="card p-5">
          ...
          {bookingsByAgent.length === 0 ? (
            <p className="text-sm text-slate-500">Pas encore assez de données pour calculer la performance.</p>
          ) : (
                ...
                return (
                  <li key={row.userId} className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{row._count} réservation(s) confirmées/payées</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-semibold text-navy">{fmtMoney(row._sum.totalPrice)}</p>
                ...
                return (
                  <li key={b.id} className="py-2.5 flex items-center justify-between gap-3">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <p className={`text-xs font-semibold ${dLeft <= 3 ? "text-red-700" : "text-amber-700"}`}>
                        J-{dLeft}
                      </p>
                    </div>
                  </li>
