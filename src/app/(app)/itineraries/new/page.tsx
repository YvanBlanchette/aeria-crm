    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Créez la fiche, puis ajoutez les escales jour par jour dans le builder.
        </p>
      </div>
      <form action={createItinerary} className="card p-6 space-y-5 max-w-2xl">
        <div>
          ...
          <input
            ...
            required
            className="input"
            placeholder="Ex. : Caraïbes de l'Est — 7 nuits"
          />
        </div>
        <div className="grid sm:grid-cols-3 gap-4">
          ...
          <div>
            <label className="label" htmlFor="departurePort">
              Port de départ *
            </label>
            <input
              id="departurePort"
          ...
          <div>
            <label className="label" htmlFor="arrivalPort">
              Port d&apos;arrivée
            </label>
            <input
              id="arrivalPort"
              name="arrivalPort"
              className="input"
              placeholder="Identique si boucle"
            />
          </div>
        </div>
        <div>
          <label className="label" htmlFor="shipId">
            Navire
          </label>
          <select id="shipId" name="shipId" className="input">
            ...
            {ships.map((s) => (
              <option key={s.id} value={s.id}>
                {s.cruiseLine.name} · {s.name}
              </option>
            ))}
          </select>
          <p className="text-xs text-slate-500 mt-1">
            Gérez compagnies et navires dans les Paramètres.
          </p>
        </div>
        <div>
