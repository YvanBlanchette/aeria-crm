      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            ...
            {itinerary.arrivalPort && itinerary.arrivalPort !== itinerary.departurePort ? ` → ${itinerary.arrivalPort}` : " (boucle)"}
      ...
      <div className="grid lg:grid-cols-2 gap-5 items-start">
        <section className="card p-5">
          <div className="flex items-center justify-between mb-5">
            ...
            <span className={`badge ${remaining > 0 ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>
          ...
          <RouteTimeline
            ...
            actions={(day) => {
              ...
              return (
                <form action={remove}>
                  <button className="text-xs text-slate-400 hover:text-red-600" title="Retirer ce jour">✕</button>
                </form>
              );
            }}
          />
          ...
          <section className="card p-5">
            ...
            <form action={addDayAction} className="space-y-4">
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" name="isSeaDay" className="rounded border-slate-300 text-ocean focus:ring-ocean" />
                Journée en mer
              </label>
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="label" htmlFor="port">Port d&apos;escale</label>
                ...
                <div>
                  <label className="label" htmlFor="arrival">Arrivée</label>
                  <input id="arrival" name="arrival" type="time" className="input" />
                </div>
                <div>
                  <label className="label" htmlFor="departure">Départ</label>
                  <input id="departure" name="departure" type="time" className="input" />
                </div>
              </div>
              <div>
                <label className="label" htmlFor="notes">Notes (excursions, conseils…)</label>
              ...
              <ul className="divide-y divide-slate-100">
                {itinerary.bookings.map((b) => (
                  <li key={b.id} className="py-2 flex justify-between text-sm">
                    <Link href={`/bookings/${b.id}`} className="text-ocean hover:underline">{b.reference}</Link>
                    <span className="text-slate-600">{b.client.firstName} {b.client.lastName}</span>
                  </li>
                ))}
              </ul>
            )}
