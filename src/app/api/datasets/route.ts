...
export async function GET(request: Request) {
  ...
  if (slug) {
    ...
    const payload = dataset.payload as {
      items?: Array<{ label?: string; value?: string; code?: string }>;
    };
...
export async function POST(request: Request) {
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    for (const file of files) {
      ...
      if (!slug) {
        return NextResponse.json(
          { ok: false, error: `Invalid slug for ${sourceFile}` },
          { status: 400 },
        );
      }
      ...
      try {
        ...
        return NextResponse.json(
          { ok: false, error: `Invalid JSON in ${sourceFile}` },
          { status: 400 },
        );
      }
