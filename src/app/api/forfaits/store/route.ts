...
export async function POST(request: Request) {
	...
	if (action === "upsertDossier") {
		...
		return NextResponse.json({ ok: true, dossier: { id, nom, ts: new Date().toISOString(), state } });
	}
	...
	if (action === "duplicateDossier") {
		...
		if (!source) return NextResponse.json({ ok: false, error: "Dossier introuvable" }, { status: 404 });
