...
async function main() {
  ...
  const admin = await prisma.user.upsert({
    where: { email: "admin@agence.local" },
    update: {},
    create: {
  ...
  if (!existingItin && ship) {
    await prisma.itinerary.create({
      data: {
        ...
        shipId: ship.id,
        description:
          "Boucle classique en Méditerranée occidentale : Espagne, France, Italie.",
        days: {
          create: [
  ...
  console.log("Seed terminé. Connexion : admin@agence.local / admin1234");
}
