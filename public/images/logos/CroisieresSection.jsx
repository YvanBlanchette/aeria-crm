  ...
  return (
    ...
    >
      <img
        ...
        onError={(e) => {
          e.target.parentElement.innerHTML = `<span style="font-size:10px;color:#666;font-weight:600;letter-spacing:0.1em;text-transform:uppercase">${compagnie}</span>`;
        }}
      />
    </div>
  );
...
function fmtPeriode(dep, ret) {
  ...
  const jD = getJour(dep),
    mD = getMois(dep),
    aD = getAnnee(dep);
  const jR = getJour(ret),
    mR = getMois(ret),
    aR = getAnnee(ret);
    ...
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <ChevronDown
            className={`size-3.5 text-stone-400 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
          />
        </div>
      </button>
...
function InclPill({ Icon, label, statut }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border font-medium ${cls}`}
    >
...
function Modal({ c, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full sm:max-w-xl bg-white sm:rounded-3xl rounded-t-3xl overflow-hidden shadow-2xl
      ...
      >
        ...
        <div className="relative h-52 overflow-hidden bg-stone-200">
          ...
          <div className="absolute bottom-0 inset-x-0 p-5">
            <h2 className="font-serif text-2xl text-white font-semibold leading-tight">
              {c["Itinéraire"]}
            </h2>
        ...
        <div className="p-5 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          {/* Prix */}
          <div>
            <p className="text-[10px] text-stone-400 tracking-[0.15em] uppercase mb-2">
              Prix par personne
            </p>
        ...
        <div className="flex items-end justify-between gap-2">
          ...
          <div className="flex flex-col items-end gap-2 shrink-0">
            <div className="flex gap-1.5">
              {c["WiFi"] === "Inclus" && (
                <Wifi className="size-3.5 text-emerald-500" title="WiFi inclus" />
              )}
              {c["Boissons"] === "Inclus" && (
                <GlassWater className="size-3.5 text-emerald-500" title="Boissons incluses" />
              )}
              {c["Pourboires"] === "Inclus" && (
                <BadgeCheck className="size-3.5 text-emerald-500" title="Pourboires inclus" />
              )}
            </div>
            <div
              className="size-8 rounded-full border border-[#B8935C]/30 flex items-center justify-center
...
const CroisieresSection = () => {
  ...
  const filtresActifs =
    fDests.length > 0 || fComps.length > 0 || fDurees.length > 0 || fMois.length > 0;
  ...
  const moisDispos = useMemo(
    () =>
      [...new Set(TOUTES.map((c) => getMois(c["Date Départ"])).filter(Boolean))].sort(
        (a, b) => a - b,
      ),
    [],
  );
  ...
  const optsMois = moisDispos.map((m) => ({
    value: String(m),
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <div className="ml-auto flex items-center gap-4">
              <p className="text-sm text-stone-500">
                <span className="font-semibold text-stone-800">{filtrees.length}</span> croisière
                {filtrees.length !== 1 ? "s" : ""}
              </p>
              {filtresActifs && (
                <button
